...
pub fn day04(input_lines: &[Vec<String>]) -> (String, String) {
    ...
    let id_of_guard_with_most_total_minutes_asleep = total_minutes_asleep_per_guard
        .iter()
        .max_by(|&(_this_guard_id, &this_guard_total), &(&_that_guard_id, &that_guard_total)| this_guard_total.cmp(&that_guard_total))
    ...
    let sleepiest_minute_of_guard_with_most_total_minutes_asleep = sleep_schedule_of_guard_with_most_total_minutes_asleep
        .iter()
        .max_by(|&(_this_minute, &this_total), &(&_that_minute, &that_total)| this_total.cmp(&that_total))
        .map(|(&minute, _total)| minute)
        .expect("Failed to find the ID of the guard who was asleep on the same minute the most.");
    let answer1 = id_of_guard_with_most_total_minutes_asleep * sleepiest_minute_of_guard_with_most_total_minutes_asleep;

    // Part 2
    let mut most_consistently_asleep_minute_and_count_per_guard = HashMap::<u32, (u32, usize)>::new();
    for (guard_id, sleep_schedule) in sleep_schedules_per_guard {
        if let Some(most_consistently_asleep_minute_and_count) = sleep_schedule
                .iter()
                .max_by(|&(_this_minute, &this_count), &(_that_minute, &that_count)| this_count.cmp(&that_count))
                .map(|(&minute, &count)| (minute, count)) {
            most_consistently_asleep_minute_and_count_per_guard.insert(guard_id, most_consistently_asleep_minute_and_count);
        }
    }
    
    let answer2 = most_consistently_asleep_minute_and_count_per_guard
        .iter()
        .max_by(|&(_this_guard_id, &(_this_guard_minute, this_guard_count)), &(_that_guard_id, &(_that_guard_minute, that_guard_count))| this_guard_count.cmp(&that_guard_count))
